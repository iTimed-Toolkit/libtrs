...
int stat_accumulate_dual_array(struct accumulator *acc, float *val0, float *val1, int len0, int len1)
{
    ...
    if(acc->acc_count == 1)
    {
        ...
        for(i = 0; i < len1;)
        {
            LOOP_HAVE_512(i, len1,
                          init_m(AVX512, &acc->acc_m.a[len0 + i], &val1[i])
            );

            LOOP_HAVE_256(i, len1,
                          init_m(AVX256, &acc->acc_m.a[len0 + i], &val1[i])
            );

            LOOP_HAVE_128(i, len1,
                          init_m(AVX128, &acc->acc_m.a[len0 + i], &val1[i])
            );
    ...
    else
    {
        ...
        for(i = 0; i < len0;)
        {
            ...
            m0_new_scalar = acc->acc_m.a[i] + (val0[i] - acc->acc_m.a[i]) / acc->acc_count;
        ...
        for(i = 0; i < len1;)
        {
                                      ...
                                      for(k = 0; k < len0;)
                                      {
                                          LOOP_HAVE_512(k, len0,
                                                        accumulate_cov(AVX512, cov, &acc->acc_cov.a[len0 * (i + j) + k],
                                                                       cov_curr, cov_m, &val0[k], &acc->acc_m.a[k]);
                                          );
                                          ...
                                          acc->acc_cov.a[len0 * (i + j) + k] +=
                                                  ((val1[i + j] * m1_new_scalar) *
                                                   (val0[k] * acc->acc_m.a[k]));
                                          k++;
                                      }
                                  }
            );
            ...
            m1_new_scalar = acc->acc_m.a[len0 + i] + (val1[i] - acc->acc_m.a[len0 + i]) / acc->acc_count;
            acc->acc_s.a[len0 + i] += ((val1[i] - acc->acc_m.a[len0 + i]) * (val1[i] - m1_new_scalar));

            IF_HAVE_128(cov_m_ = _mm_broadcast_ss(&acc->acc_m.a[len0 + i]));
            IF_HAVE_256(cov_m_256 = _mm256_broadcast_ss(&acc->acc_m.a[len0 + i]));
            IF_HAVE_512(cov_m_512 = _mm512_broadcastss_ps(cov_curr_));
            ...
            for(k = 0; k < len0;)
            {
                ...
                acc->acc_cov.a[len0 * i + k] += ((val1[i] - m1_new_scalar) *
                                                 (val0[k] - acc->acc_m.a[k]));
                k++;
            }
...
//    fprintf(stderr, "debug with count = %f\n", acc->acc_count);
//    fprintf(stderr, "\t");
//    for(i = 0; i < acc->dim0; i++)
//        fprintf(stderr, "%.5f,", acc->acc_m.a[i]);
//
//    fprintf(stderr, ",");
//    for(i = 0; i < acc->dim1; i++)
//        fprintf(stderr, "%.5f,", acc->acc_m.a[acc->dim0 + i]);
//
//    fprintf(stderr, "\n\t");
//    for(i = 0; i < acc->dim0; i++)
//        fprintf(stderr, "%.5f,", acc->acc_s.a[i]);
//
//    fprintf(stderr, ",");
//    for(i = 0; i < acc->dim1; i++)
//        fprintf(stderr, "%.5f,", acc->acc_s.a[acc->dim0 + i]);
//
//    fprintf(stderr, "\n");
...
int __get_cov_dual_array(struct accumulator *acc, int index, float *res)
{
    ...
    *res = acc->acc_cov.a[index] / acc->acc_count;
    return 0;
}
...
int __get_pearson_dual_array(struct accumulator *acc, int index, float *res)
{
    ...
    *res = acc->acc_cov.a[index] /
           ((acc->acc_count - 1) *
            sqrtf(acc->acc_s.a[i0] / (acc->acc_count - 1)) *
            sqrtf(acc->acc_s.a[acc->dim0 + i1] / (acc->acc_count - 1)));
    return 0;
}
...
int __get_mean_dual_array_all(struct accumulator *acc, float **res)
{
    ...
    memcpy(result, acc->acc_m.a, acc->dim0 * acc->dim1 * sizeof(float));
    *res = result;
    return 0;
}
...
int __get_cov_dual_array_all(struct accumulator *acc, float **res)
{
    ...
    {
        LOOP_HAVE_512(i, len,
                      reduce_cov(AVX512, &acc->acc_cov.a[i],
                                 &result[i], count);
        );

        LOOP_HAVE_256(i, len,
                      reduce_cov(AVX256, &acc->acc_cov.a[i],
                                 &result[i], count);
        );

        LOOP_HAVE_128(i, len,
                      reduce_cov(AVX128, &acc->acc_cov.a[i],
                                 &result[i], count);
        );

        result[i] = acc->acc_cov.a[i] / acc->acc_count;
        i++;
    }
...
int __get_pearson_dual_array_all(struct accumulator *acc, float **res)
{
    ...
    {
        dev = sqrtf(acc->acc_s.a[acc->dim0 + j] / count);
        ...
        for(i = 0; i < acc->dim0;)
        {
            LOOP_HAVE_512(i, acc->dim0,
                          reduce_pearson(AVX512, &acc->acc_cov.a[j * acc->dim0 + i],
                                         &acc->acc_s.a[i], dev,
                                         &result[j * acc->dim0 + i], count);
            );

            LOOP_HAVE_256(i, acc->dim0,
                          reduce_pearson(AVX256, &acc->acc_cov.a[j * acc->dim0 + i],
                                         &acc->acc_s.a[i], dev,
                                         &result[j * acc->dim0 + i], count);
            );

            LOOP_HAVE_128(i, acc->dim0,
                          reduce_pearson(AVX128, &acc->acc_cov.a[j * acc->dim0 + i],
                                         &acc->acc_s.a[i], dev,
                                         &result[j * acc->dim0 + i], count);
            );

            result[j * acc->dim0 + i] = acc->acc_cov.a[j * acc->dim0 + i] /
                                        (count * dev *
                                         sqrtf(acc->acc_s.a[i] / count));
            i++;
        }
    }
